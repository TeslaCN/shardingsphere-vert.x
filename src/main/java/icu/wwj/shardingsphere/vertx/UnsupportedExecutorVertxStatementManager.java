...
public class UnsupportedExecutorVertxStatementManager implements ExecutorVertxStatementManager {
    ...
    @Override
    public Future<Query<RowSet<Row>>> createStorageResource(final Future<? extends SqlClient> connection, final ConnectionMode connectionMode, final VertxExecutionContext option, final DatabaseType databaseType) {
        return Future.failedFuture(new UnsupportedOperationException());
    }
    
    @Override
    public Future<Query<RowSet<Row>>> createStorageResource(final ExecutionUnit executionUnit, final Future<? extends SqlClient> connection, final ConnectionMode connectionMode, final VertxExecutionContext option, final DatabaseType databaseType) {
        return Future.failedFuture(new UnsupportedOperationException());
    }
}
